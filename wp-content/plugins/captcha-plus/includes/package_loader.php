 ...
 * @package Captcha Plus by BestWebSoft
 * @since 1.1.0
 */
if ( ! class_exists( 'Cptchpls_package_loader' ) ) {
	class Cptchpls_package_loader {
		...
		function __construct() {
			...
			if( ! file_exists( $this->upload_dir ) ) {
				if ( is_writable( $upload_dir['basedir'] ) )
					...
					$this->error = __( 'Can not load images in to the "uploads" folder. Please, check your permissions.', 'captcha-plus' );
			}
		}
		...
		function parse_packages( $package_path ) {
			...
			if ( ! file_exists( $package_path . '/packages.txt' ) ) {
				$this->error = __( 'File packages.txt not found. Package not saved.', 'captcha-plus' );
				return false;
			}

			$file_content = file_get_contents( $package_path . '/' . '/packages.txt' );
			if ( ! $file_content || empty( $file_content ) ) {
				$this->error = __( 'Can not read data from packages.txt. Packages not saved.', 'captcha-plus' );
				return false;
			}
			...
			if ( ! $this->insert_data() ) {
				...
				$this->message = __( 'Package successfully loaded.', 'captcha-plus' );
				return true;
			}
		}
		...
		function insert_data() {
			...
			$update_data = $cptchpls_options['used_packages'];
			$need_update = false;
			$insert_data = array();
			...
			if ( $need_update ) {
				$cptchpls_options['used_packages']             = $update_data;
				$cptchpls_options['cptchpls_difficulty_image'] = 1;
