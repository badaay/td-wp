    ...
    public function __construct($args) {
      ...
      if (isset($args['use_with_theme'])){
        if ($args['use_with_theme'] === true){
          $this->SelfPath = get_stylesheet_directory_uri() . '/admin-settings';
        }elseif($args['use_with_theme'] === false){
          $this->SelfPath = plugins_url( 'admin-settings', plugin_basename( dirname( __FILE__ ) ) );
        }else{
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function create_export_download_link($echo = false){
    ...
    if ($echo === true)
        echo '<a href="'.esc_url($export_url).'" target="_blank">'.__('Download Export','apc').'</a>';
    elseif ($echo == 'url')
        return esc_url($export_url);
    return '<a class="button-primary" href="'.esc_url($export_url).'" target="_blank">'.__('Download Export','apc').'</a>';
  }
